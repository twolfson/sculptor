...
function process(params) {
  ...
  // TODO: Next up, flatten object
  // TODO: It might just be easier to work with commands for this one
  ...
  var template = fs.readFileSync(__dirname + '/testling/testling.template.mustache', 'utf8'),
      renderedTests = require(__dirname + '/../../testling_poc/example_test.js', 'utf8');

  // Convert all values to strings
  renderedTests.forEach(function (test) {
    test.value = test.value.toString();
  });

  var output = mustache.render(template, {'tests': renderedTests});
// console.log(output);
  // TODO: Accept more browser options
// return;
  // TODO: We might want to jsvalidate generated content pre-request
  ...
  request.put(requestOptions, function (err, res, body) {
    ...
    if (err) {
      console.error('TESTLING REQUEST ERROR:', err);
    } else {
    // Otherwise, log the body
      console.log(body);
    }
  });
}
