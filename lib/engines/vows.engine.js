    ...
    traverse(batch).forEach(function traverseBatch (node) {
      ...
      if (key) {
        ...
        if (command) {
          ...
          // Otherwise
            // TODO: Re-enable once test is built
            // // If the command is an array of commands
            // if (Array.isArray(command)) {
            //   // Convert the names into nodes
            //   var commandNamesB = command.map(function (topicObj) { return topicObj.name; }),
            //       nodeObjB = namesToNodes(commandNamesB);

            //   // Overwrite node with newNode
            //   var newNodeB = nodeObjB.root;
            //   node = newNodeB;
            // } else {
