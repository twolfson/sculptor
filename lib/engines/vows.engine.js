...
// console.log(key, node, command + '');
        // If there is a command
        if (command) {
          // If the node is an object
console.log(key, "AAA", typeof node, "BBB", command);
...
// console.log(topic);
            ...
            if (Array.isArray(topic)) {
              // TODO: This failed -- try it again once everything is back to running
              ...
              var commandNames = topic.map(function (topicObj) { return topicObj.name; }),
                  nodeArr = namesToNodes(commandNames),
                  lastIndex = commandNames.length - 1,
                  _lastNode = nodeArr[lastIndex],
                  lastName = commandNames[lastIndex],
                  lastNode = _lastNode[lastName];
...
console.log('x', require('util').inspect(batch, null, 4));
    // Return the batch
    return batch;
  }
};
