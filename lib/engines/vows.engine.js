        ...
        if (command) {
          ...
          if (typeof node === 'object') {
            ...
            if (Array.isArray(topic)) {
              // TODO: Not sure how but this logic should be moved out
              // TODO: Make just create nested objects in a pre-batch processor?
              // TODO: Move it into commandSet.get
              // Create a new node as the last node
              var subNode = {},
                  lastNode = subNode;

              // Iterate over the topic chain
              var subTopics = topic;
              subTopics.forEach(function (subTopic) {
                // Generate our next node
                var name = subTopic.name,
                    fn = subTopic.fn,
                    nextNode = {};

                // Save our next node under the last node
                lastNode[name] = nextNode;

                // Save the next last node
                lastNode = nextNode;
              });
